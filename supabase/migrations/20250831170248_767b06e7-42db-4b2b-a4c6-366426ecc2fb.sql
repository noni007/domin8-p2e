...
-- Allow users to deactivate their integrations directly
CREATE POLICY "Users can deactivate their own integrations" 
ON social_integrations 
FOR UPDATE 
USING (auth.uid() = user_id)
WITH CHECK (
  auth.uid() = user_id 
  AND (OLD.is_active = true AND NEW.is_active = false)  -- Only allow deactivation
);